...
void SampleUrlRequestCallback::OnResponseStarted(
  ...
  Cronet_UrlResponseInfo_was_cached_set(info,true);
  Cronet_UrlResponseInfo_received_byte_count_set(info,1*1024*1024* 1024);

  // Create and allocate 32kb buffer.
  Cronet_BufferPtr buffer = Cronet_Buffer_Create();
  //Cronet_Buffer_InitWithAlloc(buffer, 32 * 1024);
  Cronet_Buffer_InitWithAlloc(buffer, 10 *1024* 1024); //10M
...
void SampleUrlRequestCallback::OnReadCompleted(Cronet_UrlRequestPtr request,
  ...
  std::string fname("10M.out");
  base::FilePath file1(fname);
  int64_t size_f1 = 0;
  base::GetFileSize(file1, &size_f1);
  if( size_f1 > 10*(1<<20)  ){
